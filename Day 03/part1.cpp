...
void fillMap(vector<vector<int>>& map, int posX, int posY, int wireNumber)
{
    int currentVal = map[posY][posX];

    if (currentVal == 0)
        currentVal = wireNumber;
    else if (currentVal != wireNumber)
        currentVal = 8;
    
    map[posY][posX] = currentVal;
}

pair<int, int> getLocation(const string& code, vector<vector<int>>& map, int currentX, int currentY, int wireNumber)
{
    char opCode = code[0];
    ...
    switch (opCode)
    {
        case 'U':
            ...
            {
                fillMap(map, newX, newY - i, wireNumber);
            }
            newY -= steps;
            break;
        ...
        case 'L':
            for (int i = 1; i <= steps; i++)
            {
                fillMap(map, newX - i, newY, wireNumber);
            }
            newX -= steps;
            break;
...
void drawWires(vector<string>& wire, vector<vector<int>>& map, int wireNumber)
{
    ...
    {
        pair<int, int> location = getLocation(code, map, currentX, currentY, wireNumber);
...
void findClosestIntersection(vector<vector<int>>& map)
{
    int currentMinDist = INT_MAX;
    int originX = sizeOfGrid/2;
    int originY = sizeOfGrid/2;
    ...
    for (int y = 0; y < sizeOfGrid; y++)
    {
        for (int x = 0; x < 14000; x++)
        {
            if (map[y][x] == 8)
            {
                int dist = abs(x - originX) + abs(y - originY);
